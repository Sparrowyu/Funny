...
public class VideoUitls {
	...
	public static int[] getSize(String url) {
		...
		try {
			...
			MediaFormat mediaFormat = mediaExtractor.getTrackFormat(videoExt);
			int width = mediaFormat.containsKey(MediaFormat.KEY_WIDTH) ? mediaFormat.getInteger(MediaFormat.KEY_WIDTH) : 0;
			int height = mediaFormat.containsKey(MediaFormat.KEY_HEIGHT) ? mediaFormat.getInteger(MediaFormat.KEY_HEIGHT) : 0;
			mediaExtractor.release();
			size[0] = width;
			size[1] = height;
		} catch (Exception e) {
