 ...
 */
abstract class MySimpleCharOrderStrategy : MyCharOrderStrategy {
    ...
    override fun nextProgress(
        ...
        return MyNextProgress(currentCharIndex, offsetPercentage, previousProgress.progress)
    }

    open fun getFactor(previousProgress: MyPreviousProgress,
                       index: Int,
                       size: Int,
    ...
    open fun findCharOrder(sourceChar: Char, targetChar: Char, index: Int, order: Iterable<Char>?)
            : Pair<List<Char>, MyDirection> {
        return listOf(sourceChar, targetChar) to MyDirection.SCROLL_DOWN
    }
}
 ...
 * [SCROLL_DOWN] 向下滚动
 */
enum class MyDirection(var value: Int) {
    SCROLL_UP(-1),
    SCROLL_DOWN(1)
}