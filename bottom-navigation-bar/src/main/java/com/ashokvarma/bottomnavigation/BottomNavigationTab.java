...
abstract class BottomNavigationTab extends FrameLayout {
    ...
    protected int mFirstActiveColor = -1;
    protected boolean mActiveTextIsBold = false;
    ...
    public void setFirstActiveColor(int firstActiveColor) {
        mFirstActiveColor = firstActiveColor;
    }

    public void setActiveTextIsBold(boolean textIsBold) {
        mActiveTextIsBold = textIsBold;
    }

    public int getFirstActiveColor() {
        if (mFirstActiveColor == -1) {
            return mActiveColor;
        } else {
            return mFirstActiveColor;
        }
    }
    ...
    public void select(boolean setActiveColor, int animationDuration) {
        ...
        if (labelView != null) {
            if (setActiveColor) {
                if (getPosition() == 0) {
                    labelView.setTextColor(getFirstActiveColor());
                } else {
                    labelView.setTextColor(mActiveColor);
                }
                if (mActiveTextIsBold) {
                    labelView.setTypeface(Typeface.defaultFromStyle(Typeface.BOLD));
                }
            } else {
                if (mActiveTextIsBold) {
                    labelView.setTypeface(Typeface.defaultFromStyle(Typeface.NORMAL));
                }
    ...
    public void initialise(boolean setActiveColor) {
        ...
        if (isNoTitleMode || (labelView != null && TextUtils.isEmpty(labelView.getText()))) {
            if (labelView != null) {
