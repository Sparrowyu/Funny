...
public class ProxyCacheUtils {
    ...
    public static String getProxyUrl(String videoUrl, Map<String, String> headers, Map<String, Object> cacheParams) {
        try {
            if (VideoProxyCacheManager.getInstance().isMp4Completed(videoUrl)) {
                String path = VideoProxyCacheManager.getInstance().getMp4CachedPath(videoUrl);
                LogUtils.d(TAG, "getSavePath: " + videoUrl + " " + VideoProxyCacheManager.getInstance().isMp4Completed(videoUrl) + " " + path);
