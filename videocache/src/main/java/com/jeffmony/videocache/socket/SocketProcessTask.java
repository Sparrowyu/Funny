...
public class SocketProcessTask implements Runnable {
    ...
    private LocalProxyVideoServer.LocalProxyListener mListener;

    public SocketProcessTask(Socket socket, LocalProxyVideoServer.LocalProxyListener listener) {
        mSocket = socket;
        mListener = listener;
    }
    ...
    public void run() {
        ...
        try {
            ...
            LogUtils.i(TAG, mSocket + " sRequestCountAtomic : " + sRequestCountAtomic.get());
            while (!mSocket.isClosed() && !mSocket.isOutputShutdown() && !mSocket.isInputShutdown()) {
                request.parseRequest();
                BaseResponse response;
                String url = request.getUri();
                url = url.substring(1);
                url = ProxyCacheUtils.decodeUriWithBase64(url);
                LogUtils.w(TAG, mSocket  + " request url=" + url);
            ...
            e.printStackTrace();
            LogUtils.w(TAG, mVideoUrl + ": " + mSocket + " socket request failed, exception=" + e);
            ...
            LogUtils.w(TAG, mVideoUrl + ": " + mSocket + " finally Socket solve count = " + count);
        }
    }
}
