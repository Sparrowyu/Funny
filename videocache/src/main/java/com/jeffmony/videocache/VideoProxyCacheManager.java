            ...
            if (cacheListener != null) {
                switch (msg.what) {
                    ...
                    case ProxyMessage.MSG_VIDEO_PROXY_COMPLETED:
                        ...
                        break;
                    case ProxyMessage.MSG_VIDEO_PROXY_RANGE_COMPLETED:
                        cacheListener.onCacheRangeFinished(cacheInfo);
                        break;
                    default:
                        break;
                }
...
//        StorageManager.getInstance().initCacheInfo();
    ...
    private void startNonM3U8Task(VideoCacheInfo cacheInfo, Map<String, String> headers) {
        VideoCacheTask cacheTask = mCacheTaskMap.get(cacheInfo.getVideoUrl());
        if (cacheTask == null) {
            cacheTask = new Mp4CacheTaskForSortingHat(cacheInfo, headers);
            mCacheTaskMap.put(cacheInfo.getVideoUrl(), cacheTask);
        }
        startVideoCacheTask(cacheTask, cacheInfo);
    }
    ...
    private void startVideoCacheTask(VideoCacheTask cacheTask, VideoCacheInfo cacheInfo) {
        ...
        cacheTask.setTaskListener(new IVideoCacheTaskListener() {
            ...
            @Override
            public void onTaskRangeCompleted(float progress) {
                mProxyHandler.obtainMessage(ProxyMessage.MSG_VIDEO_PROXY_RANGE_COMPLETED, cacheInfo).sendToTarget();
            }
        });
    ...
    public void clear() {
        for(Map.Entry<String, VideoCacheTask> cacheTaskEntry : mCacheTaskMap.entrySet()) {
            VideoProxyCacheManager.getInstance().stopCacheTask(cacheTaskEntry.getKey());
            VideoProxyCacheManager.getInstance().releaseProxyReleases(cacheTaskEntry.getKey());
            OkHttpManager.getInstance().remove(cacheTaskEntry.getKey());
        }
        mCacheInfoMap.clear();
        mCacheTaskMap.clear();
    ...
    public boolean isMp4Completed(String url) {
        ...
        if (cacheInfo != null) {
            return cacheInfo.isCompleted();
        }
        return false;
    }

    public String getMp4CachedPath(String url) {
        VideoCacheInfo cacheInfo = mCacheInfoMap.get(url);
        if (cacheInfo != null) {
            return cacheInfo.getSavePath() + File.separator + cacheInfo.getMd5() + StorageUtils.NON_M3U8_SUFFIX;
        }
        return "";
    }
