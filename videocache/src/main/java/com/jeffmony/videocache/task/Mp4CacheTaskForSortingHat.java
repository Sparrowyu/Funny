 ...
 * @author jiangxiaojie
 * <p>
 * Sortingchat mp4视频缓存任务， 节省流量
 * 1、首次下载，先下载250k
 * 2、分段下载结束后，根据缓存时长决定是否继续自动下载后面的分段
 */
public class Mp4CacheTaskForSortingHat extends VideoCacheTask {

    private static final String TAG = "Mp4CacheTaskForSH";
    public static final long DEFAULT_FIRST_RANGE_SIZE = 1024 * 1024 / 4;
    public static final long CACHE_RANGE_SIZE = 1024 * 1024;
    ...
    public Mp4CacheTaskForSortingHat(VideoCacheInfo cacheInfo, Map<String, String> headers) {
    ...
    private void initVideoSegInfo() {
        if (mVideoSegMap.size() == 0) {
            //当前没有缓存,需要从头下载
            mRequestRange = new VideoRange(0, Math.min(mTotalSize, DEFAULT_FIRST_RANGE_SIZE));
        } else {
    ...
    public VideoRange getRequestRange(long position) {
        try {
            if (mVideoRangeMap.size() == 0) {
                return new VideoRange(0, Math.min(mTotalSize, DEFAULT_FIRST_RANGE_SIZE));
            } else {
                long start = -1;
                long end = -1;
                ...
                if (end == -1) {
                    end = Math.min(start + CACHE_RANGE_SIZE, mTotalSize);
                }
                return new VideoRange(start, end);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return new VideoRange(position, mTotalSize);
        }
    }
    ...
    private IMp4CacheThreadListener mCacheThreadListener = new IMp4CacheThreadListener() {
        ...
        public void onCacheRangeCompleted(VideoRange range) {
            Log.e(TAG, "onCacheRangeCompleted" + range);
    ...
    public synchronized void pauseCacheTask() {
        LogUtils.i(TAG, "pauseCacheTask: " + mVideoUrl);
...
//            mVideoCacheThread = null;
            ...
            if (!mCacheInfo.isCompleted() && mRequestRange != null) {
                ...
                LogUtils.w(TAG, "pauseCacheTask " + mVideoUrl + " " + tempRangeStart + " " + mCachedSize + " " + mTotalSize);
    ...
    public void seekToCacheTaskFromClient(float percent) {
        ...
        long endPos = (long) (mTotalSize * percent);
//        endPos = Math.min(endPos + DEFAULT_FIRST_RANGE_SIZE, mTotalSize);
        endPos = mTotalSize;//拖动进度条时如果不是全部缓存，可能播放有问题
        if (endPos <= mCachedSize) {
            return;
        }
        boolean shouldSeekToCacheTask;
        if (mVideoCacheThread != null) {
            if (mVideoCacheThread.isRunning()) {
                shouldSeekToCacheTask = shouldSeekToCacheTask(endPos);
            } else {
        ...
        LogUtils.i(TAG, "seekToCacheTaskFromClient " + percent + " shouldSeekToCacheTask=" + shouldSeekToCacheTask+", startPos = " + mCachedSize + " endPos = " + endPos);
        if (shouldSeekToCacheTask) {
            stopCacheTask();
            VideoRange requestRange = new VideoRange(mCachedSize, endPos);
            startVideoCacheThread(requestRange);
        }
    }
...
//        }
    ...
    public void resumeCacheTask() {
        LogUtils.w(TAG, "resumeCacheTask: " + mVideoUrl );
        if (mVideoCacheThread != null && mVideoCacheThread.isRunning()) {
            //当前mp4缓存线程正在运行中, 没有必要重新启动下载了
            return;
        }
        LogUtils.w(TAG, "resumeCacheTask: " + mVideoUrl + " " + mCachedSize + " " + mTotalSize);
    ...
    private void startVideoCacheThread(VideoRange requestRange) {
        ...
        LogUtils.w(TAG, "startVideoCacheThread " + mVideoUrl + " " +requestRange.getStart() + " " +  requestRange.getEnd() + " " + mTotalSize);
