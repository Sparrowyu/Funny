...
public class Mp4VideoCacheThread implements Runnable {
    ...
    private void downloadVideoByOkHttp() {
        ...
        try {
            ...
            if (!videoFile.exists()) {
                LogUtils.i("onProxyCacheInfo-noexi", "re:" + videoFile.getAbsolutePath());
                videoFile.createNewFile();
            }
            LogUtils.i("onProxyCacheInfo-mp4", "re:" + videoFile.getAbsolutePath());
        } catch (Exception e) {
        ...
        LogUtils.i("onProxyCacheInfo-range", "re:" + requestStart + "end：" + requestEnd);
...
//        }
        ...
        if (!ProxyCacheUtils.isFloatEqual(percent, mPercent)) {
            ...
            mListener.onCacheProgress(mRequestRange, cachedSize, mSpeed, percent);
            mPercent = percent;
